  ...
  <div class="reporte-detalle-container">
    ...
    <main class="main-content">
      <div class="page-header">
        ...
        <p class="subtitle">Filtrar citas por veterinario, cliente, estado o rango de fechas.</p>
      </div>

      <div class="filtros-card">
        <div class="filtro-group">
          <label>Veterinario</label>
          <select v-model="filtros.veterinario_id" class="filtro-input">
        ...
        <div class="filtro-group">
          <label>Cliente</label>
          <select v-model="filtros.cliente_id" class="filtro-input">
            <option value="">Todos</option>
        ...
        <div class="filtro-group">
          <label>Estado</label>
          <select v-model="filtros.estado" class="filtro-input">
            <option value="">Todos</option>
      ...
      <div class="resultados-container" v-else>
        <div class="resultados-header">
          <h3>{{ resultados.length }} citas encontradas</h3>
        </div>
...
const cargarListas = async () => {
  ...
  const { data: vets } = await supabase
    .from('veterinarios')
    .select('id, usuario:usuarios(nombre_completo)')
    .eq('is_activo', true)
  veterinarios.value = (vets || []).map(v => ({
    id: v.id,
    nombre: v.usuario.nombre_completo
  }))
...
const aplicarFiltros = async () => {
  ...
  try {
    const { data, error } = await supabase.rpc('obtener_citas_filtradas', {
      p_veterinario_id: filtros.value.veterinario_id || null,
      p_cliente_id: filtros.value.cliente_id || null,
      p_estado: filtros.value.estado || null,
