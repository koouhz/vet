<template>
  <div class="equipos-vet-container">
    <AppSidebar />

    <main class="main-content">
      <div class="page-header">
        <h1>Equipos Médicos</h1>
        <p class="subtitle">Inventario de equipos en la tabla <code>equiposmedicos</code>.</p>
      </div>
...
const loadEquipos = async () => {
  ...
  try {
    const { data, error: equiposError } = await supabase
      .from('equiposmedicos')
      .select('id, nombre, tipo, marca, modelo, numero_serie, fecha_compra, garantia_expira, ubicacion, estado, notas')
      .order('nombre', { ascending: true })

    if (equiposError) {
      console.error('Error al cargar equipos:', equiposError)
      throw new Error('Error al cargar el inventario de equipos')
    }
