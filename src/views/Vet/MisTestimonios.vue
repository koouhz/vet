<template>
  <div class="testimonios-vet-container">
    <AppSidebar />

    <main class="main-content">
      <div class="page-header">
        <h1>Mis Testimonios</h1>
        <p class="subtitle">Testimonios recibidos en la tabla <code>testimonios</code>.</p>
      </div>
      ...
      <div class="content-area">
        ...
        <div v-else-if="filteredTestimonios.length === 0" class="message empty">
          ...
          <p>No has recibido testimonios aún.</p>
        </div>
        ...
        <div v-else class="testimonios-grid">
          ...
          >
            ...
            <div class="card-footer">
              ...
              <div v-if="testimonio.aprobado_en" class="aprobado">
                <span class="label">Aprobado:</span>
                <span class="value">{{ formatDate(testimonio.aprobado_en) }}</span>
              </div>
            </div>
          </div>
        </div>
...
const loadTestimonios = async () => {
  ...
  try {
    ...
    let query = supabase
      .from('testimonios')
      .select('id, nombre_cliente, nombre_mascota, contenido, calificacion, publicado, destacado, creado_en, aprobado_en')
      .eq('veterinario_id', vetData.id)
      .order('creado_en', { ascending: false })

    const { data, error: testimoniosError } = await query

    if (testimoniosError) {
      console.error('Error al cargar testimonios:', testimoniosError)
      // Si el error es por columna no existente, mostrar mensaje específico
      if (testimoniosError.message.includes('column veterinario_id does not exist')) {
        throw new Error('Falta el campo veterinario_id en la tabla testimonios')
      }
      throw new Error('Error al cargar testimonios')
    }

    rawTestimonios.value = data
